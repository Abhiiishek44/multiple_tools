...
Designed for Railway's API pre-deploy command. A PostgreSQL advisory lock keeps
simultaneous deployments from racing, and stored checksums detect edits to
...
MIGRATIONS_DIRECTORY = Path(__file__).resolve().parents[1] / "infrastructure" / "migrations"
...
def _migration_body(contents: str) -> str:
    """Remove an optional outer transaction managed by an older SQL file."""
...
def apply_migrations(database_url: str, directory: Path = MIGRATIONS_DIRECTORY) -> None:
