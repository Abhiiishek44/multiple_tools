...
from packages.documents.ocr.prompts import OCR_SYSTEM_PROMPT
...
class OpenRouterOcrProvider:
    """OCR provider backed by the OpenRouter chat-completions API."""

    def __init__(self, settings: Settings) -> None:
        self.settings = settings

    def extract_text(self, source: Path, *, job_id: str | None = None) -> OcrResult:
        return extract_text(source, job_id=job_id, settings=self.settings)
...
def extract_text(
    ...
    *,
    ...
    result = _result_from_payload(_response_payload(response), configuration.openrouter_ocr_model)
    logger.info(
...
def _send_request(settings: Settings, image_url: str, job_id: str | None) -> requests.Response:
    ...
    for attempt in range(_MAX_RETRIES + 1):
        ...
        if response.status_code not in _RETRYABLE_STATUS_CODES or attempt == _MAX_RETRIES:
            raise OcrRequestError(f"OpenRouter OCR returned HTTP {response.status_code}")
...
def _encode_image(source: Path, *, maximum_pixels: int, maximum_payload_bytes: int) -> str:
    ...
    try:
        with Image.open(source) as opened:
            ...
            try:
                encoded_bytes, media_type = _encoded_content(converted, preserve_jpeg=source.suffix.lower() in {".jpg", ".jpeg"})
            finally:
                converted.close()
