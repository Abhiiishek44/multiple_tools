...
def has_usable_native_text(text: str) -> bool:
    ...
    controls = sum(unicodedata.category(character).startswith("C") for character in characters)
    replacements = text.count("\ufffd")
    return meaningful >= 3 and meaningful / len(characters) >= 0.2 and controls / len(characters) <= 0.1 and replacements / len(characters) <= 0.1
