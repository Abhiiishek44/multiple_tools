...
export function FlowSteps({ view }: { view: JobView }) {
  ...
  return <div className="relative mx-auto mt-9 grid w-[min(360px,100%)] grid-cols-3 before:absolute before:left-[16%] before:right-[16%] before:top-[13px] before:h-px before:bg-[var(--border)] before:content-['']" aria-label={`Step ${currentStep} of 3`}>{['Upload', 'Convert', 'Ready'].map((label, index) => {
    const step = index + 1
