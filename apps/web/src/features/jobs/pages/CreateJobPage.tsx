...
export function CreateJobPage({ file, tool, error, isSubmitting, onFileSelect, onRemove, onConvert }: Props) {
  return <section aria-label={`Upload a file for ${tool.name}`}><FileDropzone file={file} tool={tool} onFileSelect={onFileSelect} onRemove={onRemove} /><ToolOptionsForm key={tool.id} tool={tool} hasFile={Boolean(file)} isSubmitting={isSubmitting} error={error} onConvert={onConvert} /></section>
}
