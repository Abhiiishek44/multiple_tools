...
export function ToolPage({ tool, tools, view, children, onSelect, onHome, onTools }: Props) {
  ...
  return <main>
    ...
    <section className="mx-auto w-[min(900px,calc(100%_-_48px))] pb-[52px] pt-[58px] max-[700px]:w-[calc(100%_-_32px)]">
      <nav className="mb-7 flex flex-wrap gap-2 text-[11px] text-[var(--faint)] [&_a]:text-[var(--accent-strong)] [&_a]:no-underline" aria-label="Breadcrumb"><a href="/tools">Tools</a><span>/</span><a href={`/categories/${tool.categorySlug}`}>{tool.category}</a><span>/</span><span>{tool.name}</span></nav>
      <div className="grid grid-cols-[70px_minmax(0,1fr)_auto] items-center gap-5 text-left max-[700px]:grid-cols-[55px_minmax(0,1fr)] max-[700px]:gap-3.5"><div className={cn(toneClass(tool.category), 'grid size-[70px] place-items-center rounded-[20px] bg-[var(--tone)] max-[700px]:size-[55px] max-[700px]:rounded-2xl [&>span]:size-[54px] max-[700px]:[&>span]:size-[42px] [&_svg]:size-9 max-[700px]:[&_svg]:size-[29px]')}><ToolVisual tool={tool} /></div><div><p className={eyebrow}>{tool.category} · {tool.from} to {tool.to}</p><h1 className="m-0 text-[clamp(34px,5vw,54px)] font-[780] leading-none tracking-[-.055em] max-[700px]:text-[34px]">{tool.title}</h1><p className="mb-0 mt-3 max-w-[650px] leading-[1.55] text-[var(--muted)]">{tool.description}</p></div><button className={cn('flex h-10 cursor-pointer items-center gap-[7px] rounded-xl border border-[var(--border)] bg-[var(--surface)] px-[13px] text-[11px] text-[var(--muted)] max-[700px]:col-span-full max-[700px]:justify-self-start [&_svg]:size-[15px]', favorite && '[&_svg]:fill-[#f05252] [&_svg]:stroke-[#f05252]')} type="button" onClick={() => toggleFavorite(tool.id)} aria-label={`${favorite ? 'Remove from' : 'Add to'} favorites`}><HeartIcon /><span>{favorite ? 'Saved' : 'Save'}</span></button></div>
      <FlowSteps view={view} />
