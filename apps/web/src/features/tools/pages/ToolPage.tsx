...
export function ToolPage({ tool, children, onHome, onTools }: Props) {
  ...
  return <main>
    <Seo title={`${tool.title} | LoveMyDocument`} description={tool.description} path={canonicalPath} keywords={tool.keywords} jsonLd={jsonLd} />
    <section className="mx-auto w-[min(940px,calc(100%_-_48px))] pb-[60px] pt-[58px] max-[700px]:w-[calc(100%_-_32px)] max-[700px]:pt-10">
      <div className="grid grid-cols-[76px_minmax(0,1fr)_auto] items-center gap-5 text-left max-[700px]:grid-cols-[58px_minmax(0,1fr)] max-[700px]:gap-3.5"><ToolIcon tool={tool} size="large" className="max-[700px]:size-[58px] max-[700px]:rounded-2xl" /><div><h1 className="m-0 text-[clamp(38px,5vw,56px)] font-[800] leading-[1.02] tracking-[-.055em] max-[700px]:text-[36px]">{tool.title}</h1><p className="mb-0 mt-4 max-w-[670px] text-base leading-[1.6] text-[var(--muted)]">{tool.description}</p></div><button className={cn('favorite-button flex h-10 cursor-pointer items-center gap-[7px] rounded-lg border border-[var(--border)] bg-[var(--surface)] px-[13px] text-xs font-semibold text-[var(--muted)] transition-[color,background-color,border-color,transform] hover:border-[var(--faint)] hover:bg-[var(--surface-soft)] hover:text-[var(--text)] active:scale-95 max-[700px]:col-span-full max-[700px]:justify-self-start [&_svg]:size-[15px]', favorite && 'is-favorite border-[color-mix(in_srgb,var(--accent)_45%,var(--border))] text-[var(--accent)] [&_svg]:stroke-[var(--accent)]')} type="button" aria-pressed={favorite} onClick={() => toggleFavorite(tool.id)} aria-label={`${favorite ? 'Remove from' : 'Add to'} favorites`}><HeartIcon filled={favorite} /><span>{favorite ? 'Saved' : 'Save'}</span></button></div>
