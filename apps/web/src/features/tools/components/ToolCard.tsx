...
export function ToolCard({ tool, onSelect, variant = 'card' }: { tool: ConversionTool; onSelect: (tool: ConversionTool) => void; variant?: 'card' | 'row' }) {
  ...
  const favoriteButton = <button className={cn('grid size-[31px] cursor-pointer place-items-center rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--faint)] transition-colors hover:bg-[var(--surface-strong)] hover:text-[var(--text)] [&_svg]:size-[15px]', favorite && 'border-[color-mix(in_srgb,var(--accent)_45%,var(--border))] text-[#f05252] [&_svg]:stroke-[#f05252]')} type="button" aria-pressed={favorite} onClick={() => toggleFavorite(tool.id)} aria-label={`${favorite ? 'Remove' : 'Add'} ${tool.name} ${favorite ? 'from' : 'to'} favorites`}><HeartIcon filled={favorite} /></button>
