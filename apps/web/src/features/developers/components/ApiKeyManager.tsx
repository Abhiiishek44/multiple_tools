...
const tableGrid = 'grid grid-cols-[minmax(150px,1.2fr)_minmax(140px,1fr)_minmax(105px,.7fr)_76px_96px] items-center gap-x-3.5'
...
export function ApiKeyManager({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="mt-[30px] grid gap-3.5">
      {createdKey && (
        <div className="api-key-success rounded-xl border border-[color-mix(in_srgb,#22a866_32%,var(--border))] bg-[color-mix(in_srgb,#22a866_7%,var(--surface))] p-[18px]" role="status">
          <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-[11px]">
            <span className="grid size-8 place-items-center rounded-lg bg-[#168a53] text-white [&_svg]:w-4 [&_svg]:stroke-[2.4]"><CheckIcon /></span>
          ...
          <div className="mt-3.5 flex items-center gap-2.5 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-2 pl-[13px]"><code className="min-w-0 flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[11px] text-[var(--text)]">{createdKey}</code><button className={cn('flex min-h-[34px] cursor-pointer items-center gap-[7px] rounded-md border px-3 text-[11px] font-semibold transition-colors [&_svg]:size-3.5', copied ? 'border-[color-mix(in_srgb,#22a866_35%,var(--border))] bg-[color-mix(in_srgb,#22a866_10%,var(--surface))] text-[#168a53]' : 'border-[var(--border)] bg-[var(--surface-soft)] text-[var(--text)] hover:bg-[var(--surface-strong)]')} type="button" onClick={() => void copy()}>{copied ? <CheckIcon /> : <CopyIcon />}{copied ? 'Copied' : 'Copy key'}</button></div>
        </div>
      )}
        ...
        <div className="border-y border-[var(--border)]" role="table" aria-label="API keys">
          ...
          {keys.length === 0 ? (
            ...
            <article className={`${tableGrid} min-h-[78px] border-t border-[var(--border)] px-5 py-[13px] first:border-t-0 max-[700px]:grid-cols-[minmax(0,1fr)_auto] max-[700px]:gap-3 max-[700px]:p-4 [&>div]:min-w-0`} role="row" key={key.key_id}>
              ...
              <div className="max-[700px]:col-start-2 max-[700px]:row-start-1 max-[700px]:self-start" role="cell"><span className={cn('inline-flex min-h-7 items-center gap-1.5 rounded-md border border-[color-mix(in_srgb,#22a866_28%,var(--border))] bg-[color-mix(in_srgb,#22a866_8%,var(--surface))] px-2 text-[10px] font-semibold text-[#198954] [&_i]:size-1.5 [&_i]:rounded-full [&_i]:bg-[#24b36b]', key.revoked_at && 'border-[var(--border)] bg-[var(--surface-soft)] text-[var(--faint)] [&_i]:bg-[var(--faint)]')}><i />{key.revoked_at ? 'Revoked' : 'Active'}</span></div>
              <div className="justify-self-end whitespace-nowrap text-right max-[700px]:col-start-2 max-[700px]:row-start-2 max-[700px]:row-span-2 max-[700px]:self-center" role="cell">{!key.revoked_at && <button className="inline-flex min-h-[34px] cursor-pointer items-center gap-1.5 rounded-lg border border-[color-mix(in_srgb,var(--danger)_38%,var(--border))] bg-transparent px-2.5 text-[11px] font-bold text-[var(--danger)] transition-colors hover:bg-[color-mix(in_srgb,var(--danger)_8%,transparent)] disabled:cursor-wait disabled:opacity-55 [&_svg]:w-3" type="button" disabled={revokingKeyId === key.key_id} onClick={() => setKeyToRevoke(key)} aria-label={`Revoke ${key.name}`}><TrashIcon />{revokingKeyId === key.key_id ? 'Revoking…' : 'Revoke'}</button>}</div>
            </article>
          ))}
        </div>
      ...
      {isCreateOpen && (
        <div className="dialog-backdrop fixed inset-0 z-120 grid place-items-center overflow-y-auto bg-[rgba(7,8,7,.58)] p-6 backdrop-blur-[4px] max-[700px]:items-end max-[700px]:p-0" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setIsCreateOpen(false) }}>
          <section className="dialog-panel max-h-[calc(100vh-48px)] w-[min(620px,100%)] overflow-y-auto rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-[0_24px_70px_rgba(0,0,0,.28)] max-[700px]:max-h-[92vh] max-[700px]:w-full max-[700px]:rounded-b-none" role="dialog" aria-modal="true" aria-labelledby={dialogTitleId} aria-busy={busy}>
            ...
            <form className="p-0" onSubmit={(event) => { event.preventDefault(); void create() }}>
              <div className="p-[22px] max-[700px]:px-[18px] max-[700px]:py-5">
                ...
                <label><span className="block text-sm font-[650] text-[var(--text)]">Name</span><input className="mt-2 h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--surface-soft)] px-3 text-[13px] text-[var(--text)] outline-0 placeholder:text-[var(--faint)] focus:border-[var(--accent-strong)] focus:bg-[var(--surface)] focus:shadow-[0_0_0_3px_color-mix(in_srgb,var(--accent)_18%,transparent)] disabled:cursor-wait disabled:opacity-60" autoFocus disabled={busy} type="text" value={name} maxLength={100} onChange={(event) => setName(event.target.value)} placeholder="e.g. Production backend" /></label>
                <fieldset className="mb-0 mt-[17px] border-0 p-0 disabled:cursor-wait disabled:opacity-60" disabled={busy}><legend className="mb-[7px] block text-sm font-[650] text-[var(--text)]">Permissions</legend><div className="flex flex-col gap-2">{SCOPE_OPTIONS.map((scope) => <label className="relative grid w-fit max-w-full cursor-pointer grid-cols-[17px_minmax(0,1fr)] items-start gap-[9px]" key={scope.value}><input className="peer absolute opacity-0" type="checkbox" checked={scopes.includes(scope.value)} onChange={() => toggleScope(scope.value)} /><span className="mt-px grid size-[17px] place-items-center rounded border border-[var(--border)] bg-[var(--surface-soft)] text-transparent peer-checked:border-[var(--accent-strong)] peer-checked:bg-[var(--accent)] peer-checked:text-[var(--accent-text)] [&_svg]:w-[11px] [&_svg]:stroke-[2.5]"><CheckIcon /></span><span className="min-w-0"><strong className="block text-[13px] font-bold">{scope.value}</strong><small className="mt-[3px] block text-xs leading-[1.35] text-[var(--muted)]">{scope.description}</small></span></label>)}</div></fieldset>
              </div>
              <footer className="flex min-h-[66px] items-center justify-end gap-2 border-t border-[var(--border)] bg-[var(--surface-soft)] px-[22px] py-3 max-[700px]:px-[18px]"><button className="inline-flex h-10 min-w-[78px] cursor-pointer items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--surface)] px-[15px] text-xs font-semibold text-[var(--text)] transition-colors hover:bg-[var(--surface-strong)] disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={busy} onClick={() => setIsCreateOpen(false)}>Cancel</button><button className="inline-flex h-10 min-w-[118px] cursor-pointer items-center justify-center gap-2 rounded-lg border border-[var(--accent)] bg-[var(--accent)] px-[15px] text-xs font-semibold text-[var(--accent-text)] transition-colors hover:border-[var(--accent-strong)] hover:bg-[var(--accent-strong)] disabled:cursor-not-allowed disabled:opacity-70" type="submit" disabled={busy || !name.trim() || scopes.length === 0}>{busy && <span className="size-3.5 animate-spin rounded-full border-2 border-white/35 border-t-white" aria-hidden="true" />}{busy ? 'Creating key…' : 'Create API key'}</button></footer>
            </form>
          </section>
        </div>
      )}
