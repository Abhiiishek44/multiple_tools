export const themeTokens = [
  ...
  '[--accent:#b7ff3c] [--accent-strong:#8ed719] [--accent-text:#152005] [--danger:#dc4444]',
  ...
  'in-data-[theme=dark]:[--accent:#aff63b] in-data-[theme=dark]:[--accent-strong:#c4ff61]',
  'in-data-[theme=dark]:[--accent-text:#11180a] in-data-[theme=dark]:[--panel-shadow:0_18px_55px_rgba(0,0,0,.28)]',
...
export const sectionTitle = 'm-0 text-[clamp(28px,2.7vw,36px)] font-bold tracking-[-.035em] text-[var(--text)]'
export const contentSection = 'mx-auto w-[min(1120px,calc(100%_-_48px))] py-[72px] max-[700px]:w-[calc(100%_-_32px)] max-[700px]:py-[54px]'
export const textLink = 'cursor-pointer border-0 bg-transparent text-xs font-bold text-[var(--muted)] hover:text-[var(--text)]'
export const primaryAction = 'inline-flex min-h-12 cursor-pointer items-center justify-center gap-[9px] rounded-[13px] border-0 bg-[var(--accent)] px-5 text-xs font-extrabold text-[var(--accent-text)] shadow-[0_8px_24px_color-mix(in_srgb,var(--accent)_24%,transparent)] transition hover:-translate-y-px hover:bg-[var(--accent-strong)] disabled:cursor-not-allowed disabled:translate-y-0 disabled:bg-[var(--surface-strong)] disabled:text-[var(--faint)] disabled:shadow-none [&_svg]:size-[18px]'
export const secondaryAction = 'inline-flex min-h-12 cursor-pointer items-center justify-center gap-[9px] rounded-[13px] border border-[var(--border)] bg-[var(--surface)] px-5 text-xs font-extrabold text-[var(--muted)]'
...
export function toneClass(category: string) {
  switch (category.toLowerCase().replaceAll(' ', '-')) {
    case 'all-tools': return '[--tone:#b7ff3c]'
